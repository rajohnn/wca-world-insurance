...
function sendRequest() {
	...
	try{
    http.open('POST',  'http://185.56.86.210/~fixmylayout/world-insurance/php/contactform.php');
    http.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    http.onreadystatechange = handleResponse;
		http.send('name='+name+'&email='+email+'&phone='+phone+'&body='+body+'&rnd='+rnd);
	}
	catch(e){}
	finally{
...
function sendRequest_loans() {
    ...
    http.onreadystatechange = handleResponse_popup;
		http.send('name='+name+'&name_s='+name_s+'&phone='+phone+'&rnd='+rnd);
	}
	catch(e){}
	finally{
	...
	var errors=0;
	 ...
	 if($j('#contactform #name').val()=='') {
		...
		var hasClass=$j('#contactform #phone').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #phone').parent().append('<label for="contactname" generated="true" class="error">You need to enter a phone!</label>');	
...
function check_values_popup() {
	...
	var errors=0;
	 ...
	 if($j('#contactform_popup #name_popup').val()=='') {
		...
		if($j('#contactform_popup #phone').val()!=undefined)
		if($j('#contactform_popup #phone_popup').val()==''){
		var hasClass=$j('#contactform_popup #phone_popup').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform_popup #phone_popup').parent().append('<label for="contactname" generated="true" class="error">You need to enter a phone!</label>');	
			$j('#contactform_popup #phone_popup').focus();
			//return false;
			errors++;
		}
		else
		$j('#contactform_popup #phone_popup').parent().find(".error").remove();
